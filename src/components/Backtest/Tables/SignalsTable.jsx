      ...
      <table className="signal-table">
        <thead>
          <tr className="signal-table-header-row">
            ...
            <th className="signal-table-header">Signal ID</th>
            ...
            <React.Fragment key={index}>
              {trade.trade_instructions.map((instruction, idx) => (
                <tr className="signal-table-row" key={idx}>
                  {idx === 0 && (
                    <td
                      className="signal-timestamp-column-data"
                      rowSpan={trade.trade_instructions.length}
                    >
                      {trade.iso_timestamp}
                    </td>
                  )}
                  <td className="signal-table-data">{instruction.signal_id}</td>
