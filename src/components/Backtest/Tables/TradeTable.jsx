...
function TradeTable({ trades_data }) {
  const { currentItems, goToNextPage, goToPreviousPage, currentPage, totalPages } = usePagination(trades_data, 12);
      ...
      <table className='trade-table'>
        <thead>
          <tr className='trade-table-header-row'>
            ...
            <th className='trade-table-header'>Leg ID</th>
        ...
        <tbody>
          {currentItems.map((trade, index) => (
            <tr className='trade-table-row' key={index}>
              <td className='trade-timestamp-column-data'>{trade.iso_timestamp}</td>
